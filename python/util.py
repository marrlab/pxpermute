...
def read_config_file(config_file_path):
    ...
    parameters["device"] = general_section.get("device").strip(
        "'"
    )  # Remove single quotes

    # param.data section
    data_section = config["param.data"]
    parameters["data_path"] = data_section.get("data_path").strip(
        "'"
    )  # Remove single quotes
    ...
    parameters["selected_channels"] = ast.literal_eval(
        data_section.get("selected_channels")
    )
    ...
    assert parameters["num_channels"] == len(parameters["channel_names"]), (
        "the legth of selected_channels does not " "match with channel_names "
    )

    # param.model section
    model_section = config["param.model"]
    parameters["model_path"] = model_section.get("model_path").strip(
        "'"
    )  # Remove single quotes
    ...
    parameters["require_baseline"] = interpretation_section.getboolean(
        "require_baseline", fallback=False
    )
    parameters["require_sliding_window"] = interpretation_section.getboolean(
        "require_sliding_window", fallback=False
    )

    # Check for sliding_window_shapes (optional)
    if "sliding_window_shapes" in data_section:
        parameters["sliding_window_shapes"] = ast.literal_eval(
            interpretation_section.get("sliding_window_shapes")
        )
...
def prepare_test_data(
    ...
    **kwargs
    ...
    if len(test_index) == 0:
        ...
        try:
            _, test_index, _, _ = train_test_split(
                index_to_split,
    ...
    label_map = dict(
        zip(
            sorted(set(metadata.loc[test_index, "label"])),
